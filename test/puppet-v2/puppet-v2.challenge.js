...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        console.log("WETH deposit required [before]: ",(await this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE)).toString()/10**18);

        var path = [];
        path[0] = this.token.address;
        path[1] = await this.uniswapRouter.WETH();
        ...
        console.log("WETH deposit required  [after]: ",(await this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE)).toString()/10**18);
        console.log("Attacker's balance:             ",(await ethers.provider.getBalance(attacker.address)).toString()/10**18);


        var WethDeposit = (await this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE)).toString()
        ...
        console.log("---------- Tokens -------------");
        console.log("Attacker's balance:   ",(await this.token.balanceOf(attacker.address)).toString()/10**18);
        console.log("Lending pool balance: ", (await this.token.balanceOf(this.lendingPool.address)).toString()/10**18);
        console.log("---------- WETH -------------");
        console.log("Attacker's balance:   ",(await ethers.provider.getBalance(attacker.address)).toString()/10**18);
        console.log("Lending pool balance: ", (await ethers.provider.getBalance(this.lendingPool.address)).toString()/10**18);
